...
namespace Parse.Platform.LiveQueries
{
    ...
    public class ParseLiveQueryController : IParseLiveQueryController, IDisposable, IAsyncDisposable
    {
        ...
        TaskCompletionSource<bool> ConnectionSignal { get; set; }
        private ConcurrentDictionary<int, TaskCompletionSource<bool>> SubscriptionSignals { get; } = new ConcurrentDictionary<int, TaskCompletionSource<bool>>();
        private ConcurrentDictionary<int, TaskCompletionSource<bool>> UnsubscriptionSignals { get; } = new ConcurrentDictionary<int, TaskCompletionSource<bool>>();
        ...
        void ProcessUnsubscriptionMessage(IDictionary<string, object> message)
        {
            ...
            if (UnsubscriptionSignals.TryGetValue(requestId, out TaskCompletionSource<bool> unsubscriptionSign))
            {
                unsubscriptionSign?.TrySetResult(true);
            }
        }
        ...
        void ProcessSubscriptionMessage(IDictionary<string, object> message)
        {
            ...
            if (SubscriptionSignals.TryGetValue(requestId, out TaskCompletionSource<bool> subscriptionSignal))
            {
                subscriptionSignal?.TrySetResult(true);
            }
        }
        ...
        void ProcessConnectionMessage(IDictionary<string, object> message)
        {
            ...
            ConnectionSignal?.TrySetResult(true);
        }
        ...
        public async Task ConnectAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (_state == ParseLiveQueryState.Closed)
            {
                ...
                ConnectionSignal = new TaskCompletionSource<bool>();
                try
                {
                    ...
                    var waitTask = ConnectionSignal.Task;
                    using (cancellationToken.Register(() => ConnectionSignal.TrySetCanceled(), useSynchronizationContext: false))
                    {
            ...
            {
                TaskCompletionSource<bool> signal = ConnectionSignal;
                if (signal is not null)
                {
                    // await signal.Task.WaitAsync(cancellationToken);
                    var waitTask = signal.Task;
                    using (cancellationToken.Register(() => signal.TrySetCanceled(), useSynchronizationContext: false))
                    {
                        await waitTask.ConfigureAwait(false);
                    }
                }
        ...
        private async Task SendAndWaitForSignalAsync(IDictionary<string, object> message,
            ConcurrentDictionary<int, TaskCompletionSource<bool>> signalDictionary,
            int requestId,
        ...
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(ParseLiveQueryController));
        }
    }
}